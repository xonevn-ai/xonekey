...
- (void)viewDidLoad {
    ...
    NSArray* codeData = [XoneKeyManager getTableCodes];
...
- (void)viewDidAppear {
    [super viewDidAppear];
    NSString* str = @"XoneKey %@ - Bộ gõ Tiếng Việt";
...
- (IBAction)onTempOffXoneKeyByHotKey:(id)sender {
    NSInteger val = [self setCustomValue:sender keyToSet:@"vTempOffXoneKey"];
    vTempOffXoneKey = (int)val;
}
...
-(void)fillData {
    ...
    value = [[NSUserDefaults standardUserDefaults] integerForKey:@"vTempOffXoneKey"];
    self.TempOffXoneKey.state = value ? NSControlStateValueOn : NSControlStateValueOff;
...
- (IBAction)onHomePageLink:(id)sender {
    [[NSWorkspace sharedWorkspace] openURL: [NSURL URLWithString:@"https://vkey.xone.vn"]];
}
...
- (IBAction)onEmailLink:(id)sender {
    [[NSWorkspace sharedWorkspace] openURL: [NSURL URLWithString:@"mailto:diep@xone.vn"]];
}
