...
int vTempOffXoneKey = 0; //new on version 2.0
...
-(void)askPermission {
    ...
    [alert setMessageText: [NSString stringWithFormat:@"XoneKey cần bạn cấp quyền để có thể hoạt động!"]];
...
- (void)applicationDidFinishLaunching:(NSNotification *)aNotification {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        if (![XoneKeyManager initEventTap]) {
    ...
    if (!dontCheckUpdate)
        [XoneKeyManager checkNewVersion:nil callbackFunc:nil];
...
-(void)loadDefaultConfig {
    ...
    vTempOffXoneKey = 0;[[NSUserDefaults standardUserDefaults] setInteger:vTempOffXoneKey forKey:@"vTempOffXoneKey"];
...
-(void)setRunOnStartup:(BOOL)val {
    CFStringRef appId = (__bridge CFStringRef)@"com.xonevn.XoneKeyHelper";
...
-(void)onConvertTool {
    ...
    //[XoneKeyManager showDockIcon:YES];
...
-(void)onQuickConvert {
    if ([XoneKeyManager quickConvert]) {
        if (!convertToolDontAlertWhenCompleted) {
            [XoneKeyManager showMessage: nil message:@"Chuyển mã thành công!" subMsg:@"Kết quả đã được lưu trong clipboard."];
        }
    } else {
        [XoneKeyManager showMessage: nil message:@"Không có dữ liệu trong clipboard!" subMsg:@"Hãy sao chép một đoạn text để chuyển đổi!"];
    }
}

-(void) onControlPanelSelected {
    if (_mainWC == nil) {
        _mainWC = [[NSStoryboard storyboardWithName:@"Main" bundle:nil] instantiateControllerWithIdentifier:@"XoneKey"];
    }
    //[XoneKeyManager showDockIcon:YES];
...
#pragma mark Reset XoneKey after mac computer awake
-(void)receiveWakeNote: (NSNotification*)note {
    [XoneKeyManager initEventTap];
}
...
-(void)activeAppChanged: (NSNotification*)note {
    if (vUseSmartSwitchKey && [XoneKeyManager isInited]) {
        OnActiveAppChanged();
    }
}
