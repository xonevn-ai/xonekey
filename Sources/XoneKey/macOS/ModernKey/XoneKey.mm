...
// Check if other control keys are pressed
static inline bool HasOtherControlKey(CGEventFlags flag) {
    return (flag & kCGEventFlagMaskCommand) || 
           ...
           (flag & kCGEventFlagMaskNumericPad) || 
           (flag & kCGEventFlagMaskHelp);
}

// Get data from macro or char array safely
static inline Uint32 GetDynaData(bool isMacro, int pos, vKeyHookState* data) {
    if (!data) return 0;
    if (isMacro) {
        if (pos >= 0 && pos < (int)data->macroData.size()) {
            return data->macroData[pos];
        }
    } else {
        if (pos >= 0 && pos < MAX_BUFF) {
            return data->charData[pos];
        }
    }
    return 0;
}
...
static inline void LoadData(int& var, NSString* key, int defaultValue = 0, int minValue = INT_MIN, int maxValue = INT_MAX) {
    NSUserDefaults* defaults = [NSUserDefaults standardUserDefaults];
    int value = (int)[defaults integerForKey:key];
    // Validate and clamp value
    if (value < minValue) value = defaultValue;
    if (value > maxValue) value = defaultValue;
    var = value;
}
...
extern "C" {
    ...
    void XoneKeyInit() {
        ...
        vFreeMark = 0; // Not used currently
        
        // Load with validation - vCodeTable should be 0-4
        LoadData(vCodeTable, @"CodeTable", 0, 0, 4);
        
        // Boolean flags (0 or 1)
        LoadData(vCheckSpelling, @"Spelling", 1, 0, 1);
        LoadData(vQuickTelex, @"QuickTelex", 0, 0, 1);
        LoadData(vUseModernOrthography, @"ModernOrthography", 1, 0, 1);
        LoadData(vRestoreIfWrongSpelling, @"RestoreIfInvalidWord", 0, 0, 1);
        LoadData(vFixRecommendBrowser, @"FixRecommendBrowser", 1, 0, 1);
        LoadData(vUseMacro, @"UseMacro", 1, 0, 1);
        LoadData(vUseMacroInEnglishMode, @"UseMacroInEnglishMode", 1, 0, 1);
        LoadData(vAutoCapsMacro, @"vAutoCapsMacro", 0, 0, 1);
        LoadData(vSendKeyStepByStep, @"SendKeyStepByStep", 0, 0, 1);
        LoadData(vUseSmartSwitchKey, @"UseSmartSwitchKey", 1, 0, 1);
        LoadData(vUpperCaseFirstChar, @"UpperCaseFirstChar", 0, 0, 1);
        
        LoadData(vTempOffSpelling, @"vTempOffSpelling", 0, 0, 1);
        LoadData(vAllowConsonantZFWJ, @"vAllowConsonantZFWJ", 0, 0, 1);
        LoadData(vQuickEndConsonant, @"vQuickEndConsonant", 0, 0, 1);
        LoadData(vQuickStartConsonant, @"vQuickStartConsonant", 0, 0, 1);
        LoadData(vRememberCode, @"vRememberCode", 1, 0, 1);
        LoadData(vOtherLanguage, @"vOtherLanguage", 1, 0, 1);
        LoadData(vTempOffXoneKey, @"vTempOffXoneKey", 0, 0, 1);
        
        LoadData(vFixChromiumBrowser, @"vFixChromiumBrowser", 0, 0, 1);
        LoadData(vPerformLayoutCompat, @"vPerformLayoutCompat", 0, 0, 1);
    ...
    void queryFrontMostApp() {
        if ([[[NSWorkspace sharedWorkspace] frontmostApplication].bundleIdentifier compare:XONEKEY_BUNDLE] != 0) {
    ...
    void SendEmptyCharacter() {
        ...
        if ([_niceSpaceApp containsObject:GetFrontMostApp()]) {
    ...
    void SendNewCharString(const bool& dataFromMacro=false, const Uint16& offset=0) {
        ...
        if (_newCharSize > 0) {
            for (_k = dataFromMacro ? offset : pData->newCharCount - 1 - offset;
                ...
                _tempChar = GetDynaData(dataFromMacro, _k, pData);
     ...
     * MAIN Callback.
     */
    CGEventRef XoneKeyCallback(CGEventTapProxy proxy, CGEventType type, CGEventRef event, void *refcon) {
        ...
        if (type == kCGEventKeyDown) {
            ...
            if (_lastFlag == 0 || _lastFlag < _flag) {
                ...
                if (vTempOffXoneKey && !_hasJustUsedHotKey && _lastFlag & kCGEventFlagMaskCommand) {
                    vTempOffEngine();
                }
                _lastFlag = 0;
            ...
            if (pData->code == vDoNothing) { //do nothing
                if (IS_DOUBLE_CODE(vCodeTable)) { //VNI
                    if (pData->extCode == 1) { //break key
                        ...
                        if (_syncKey.size() > 0) {
                            if (_syncKey.back() > 1 && (vCodeTable == 2 || !containUnicodeCompoundApp(GetFrontMostApp()))) {
                ...
                if (vFixRecommendBrowser && pData->extCode != 4) {
                    if (vFixChromiumBrowser && [_unicodeCompoundApp containsObject:GetFrontMostApp()]) {
                        if (pData->backspaceCount > 0) {
