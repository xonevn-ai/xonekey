...
void MainControlDialog::initDialog() {
    ...
    wsprintfW(titleBuffer, title, XoneKeyHelper::getVersionString().c_str());
    SetWindowText(hDlg, titleBuffer);

    //input type
    comboBoxInputType = GetDlgItem(hDlg, IDC_COMBO_INPUT_TYPE);
    vector<LPCTSTR>& inputType = XoneKeyManager::getInputType();
    ...
    vector<LPCTSTR>& tableCode = XoneKeyManager::getTableCode();
...
// Update the eventProc method to handle WM_USER+1001 messages
INT_PTR MainControlDialog::eventProc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_INITDIALOG:
        ...
        initDialog();
        return TRUE;
    case WM_USER + 1001: // Update check result message
        handleUpdateCheckResult(wParam, lParam);
        return TRUE;
    case WM_COMMAND: {
        ...
        default:
            if (HIWORD(wParam) == CBN_SELCHANGE) {
                ...
                _lastKeyCode = XoneKeyManager::_lastKeyCode;
                if (_lastKeyCode > 0) {
                    XoneKeyManager::_lastKeyCode = 0;
    ...
    case WM_NOTIFY: {
        ...
        case NM_RETURN: {
            ...
            if (link->hdr.idFrom == IDC_SYSLINK_HOME_PAGE)
                ShellExecute(NULL, _T("open"), _T("http://vkey.xone.vn"), NULL, NULL, SW_SHOWNORMAL);
                ...
                ShellExecute(NULL, _T("open"), _T("mailto:diep@xone.vn"), NULL, NULL, SW_SHOWNORMAL);
            break;
        }
        }
        break;
...
void MainControlDialog::fillData() {
    ...
    SendMessage(checkTempOffXoneKey, BM_SETCHECK, vTempOffXoneKey ? 1 : 0, 0);
...
void MainControlDialog::onCheckboxClicked(const HWND& hWnd) {
    ...
    if (hWnd == checkCtrl) {
        ...
        if (val)
            XoneKeyManager::createDesktopShortcut();
    }
    ...
    else if (hWnd == checkTempOffXoneKey) {
        val = (int)SendMessage(hWnd, BM_GETCHECK, 0, 0);
        APP_SET_DATA(vTempOffXoneKey, val ? 1 : 0);
    }
...
void MainControlDialog::onUpdateButton() {
    ...
    // Show an info toast while checking
    ToastNotification::Show(hDlg, _T("Đang kiểm tra bản mới..."), TOAST_INFO);
    
    // Start asynchronous update check
    XoneKeyHelper::StartAsyncUpdateCheck(hDlg, hUpdateButton);
}

void MainControlDialog::handleUpdateCheckResult(WPARAM wParam, LPARAM lParam) {
    EnableWindow(hUpdateButton, true);
    
    if (wParam == 1) { // Update available
        ...
        if (lParam) free((char*)lParam); // Free if allocated but unused
        MessageBox(hDlg, _T("Bạn đang dùng phiên bản mới nhất!"), _T("XoneKey Update"), MB_OK);
    }
}

void MainControlDialog::requestRestartAsAdmin() {
    XoneKeyHelper::registerRunOnStartup(false);
    if (vRunAsAdmin && !IsUserAnAdmin()) {
        int msgboxID = MessageBox(
            hDlg,
            _T("Bạn cần phải khởi động lại XoneKey để kích hoạt chế độ Admin!\nBạn có muốn khởi động lại XoneKey không?"),
            _T("XoneKey"),
            MB_ICONEXCLAMATION | MB_YESNO
        );
        if (msgboxID == IDYES) {
