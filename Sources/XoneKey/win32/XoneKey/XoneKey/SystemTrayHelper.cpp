...
map<UINT, LPCTSTR> menuData = {
	...
	{POPUP_ABOUT_OPENKEY, _T("Giới thiệu XoneKey")},
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_USER + 2019:
		...
		break;
	case WM_USER + 2020:
		// Update check completed, show dialog
		if (lParam != 0)
		{
...
void SystemTrayHelper::updateData()
{
	try
	{
		...
		if (nid.hWnd != NULL)
		{
			...
			{
				// If modification fails, try to re-add the icon
				bool reAddSuccess = Shell_NotifyIcon(NIM_ADD, &nid) != FALSE;
				PerformanceLogger::LogSystemTrayUpdate(reAddSuccess);
			}
			else
			{
				PerformanceLogger::LogSystemTrayUpdate(true);
			}
		}
		else
		{
			PerformanceLogger::LogSystemTrayUpdate(false);
		}
		...
		// Update the quick convert hotkey display
		wstring hotkey = L"";
		bool hasAdd = false;
...
void SystemTrayHelper::_createSystemTrayIcon(const HINSTANCE &hIns)
{
	...
	{ // Use timer to create
		if (recreateCount >= 5)
		{
			MessageBox(NULL, _T("Failed to create system tray icon after several attempts."),
					   _T("XoneKey Error"), MB_OK | MB_ICONERROR);
			PostQuitMessage(0);
			return;
		}
		ins = hIns;
	...
	try
	{
		...
		// Attempt to add the notification icon with retries
		...
		if (!success)
		{
			MessageBox(NULL, _T("Failed to create system tray icon. Try running as administrator."),
					   _T("XoneKey Warning"), MB_OK | MB_ICONWARNING);
		}
	}
	catch (const std::exception &)
	{
		MessageBox(NULL, _T("Error initializing system tray icon."),
				   _T("XoneKey Error"), MB_OK | MB_ICONERROR);
	}
}
...
{
	// Only try to remove if we have a valid window handle
	if (nid.hWnd != NULL)
	{
