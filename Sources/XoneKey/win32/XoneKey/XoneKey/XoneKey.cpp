...
static DWORD _lastWindowEventTime = 0;
...
void XoneKeyFree() {
	...
	DeleteCriticalSection(&_windowEventCs);
}
...
void XoneKeyInit() {
	...
	XoneKeyHelper::registerRunOnStartup(vRunWithWindows);
	...
	APP_GET_DATA(vTempOffXoneKey, 0);
	...
	DWORD macroDataSize;
	BYTE* macroData = XoneKeyHelper::getRegBinary(_T("macroData"), macroDataSize);
	initMacroMap((Byte*)macroData, (int)macroDataSize);

	//init and load smart switch key data
	DWORD smartSwitchKeySize;
	BYTE* data = XoneKeyHelper::getRegBinary(_T("smartSwitchKey"), smartSwitchKeySize);
	initSmartSwitchKey((Byte*)data, (int)smartSwitchKeySize);

	// Initialize critical section for window event throttling
	InitializeCriticalSection(&_windowEventCs);
...
void saveSmartSwitchKeyData() {
	...
	XoneKeyHelper::setRegBinary(_T("smartSwitchKey"), savedSmartSwitchKeyData.data(), (int)savedSmartSwitchKeyData.size());
}
...
static void SendBackspace() {
	...
	if (vSupportMetroApp && XoneKeyHelper::getLastAppExecuteName().compare("ApplicationFrameHost.exe") == 0) {//Metro App
		...
		// SendMessage(HWND_BROADCAST) can hang if any window doesn't respond
...
LRESULT CALLBACK keyboardHookProcess(int nCode, WPARAM wParam, LPARAM lParam) {
	...
	if ((wParam == WM_KEYDOWN || wParam == WM_SYSKEYDOWN) && !_isFlagKey && _keycode != 0) {
		...
		if (_lastFlag == 0 || _lastFlag < _flag)
			...
			if (vTempOffXoneKey && !_hasJustUsedHotKey && _lastFlag & MASK_ALT) {
				vTempOffEngine();
			}
			_lastFlag = _flag;
		...
		if (pData->code == vDoNothing) { //do nothing
			...
			if (vFixRecommendBrowser && pData->extCode != 4) {
				if (vFixChromiumBrowser && 
					std::find(_chromiumBrowser.begin(), _chromiumBrowser.end(), XoneKeyHelper::getLastAppExecuteName()) != _chromiumBrowser.end()) {
...
VOID CALLBACK winEventProcCallback(HWINEVENTHOOK hWinEventHook, DWORD dwEvent, HWND hwnd, LONG idObject, LONG idChild, DWORD dwEventThread, DWORD dwmsEventTime) {
	...
	// Throttle window events to prevent excessive processing
	EnterCriticalSection(&_windowEventCs);
	DWORD currentTime = GetTickCount();
	DWORD timeSinceLastEvent = currentTime - _lastWindowEventTime;
	
	if (timeSinceLastEvent < WINDOW_EVENT_THROTTLE_MS)
	{
	...
	if (vUseSmartSwitchKey || vRememberCode) {
		...
		if (vUseSmartSwitchKey && (_languageTemp & 0x01) != vLanguage) {
			if (_languageTemp != -1) {
				...
				// Don't show toast for automatic Smart Switch Key changes
				// This prevents annoying notifications when switching between apps
				AppDelegate::getInstance()->onInputMethodChangedFromHotKey(false);
			} else {
				saveSmartSwitchKeyData();
			}
		}
		...
		if (vRememberCode && (_languageTemp >> 1) != vCodeTable) { //for remember table code feature
			if (_languageTemp != -1) {
				// Don't show toast when called from hook thread (automatic change)
				AppDelegate::getInstance()->onTableCode(_languageTemp >> 1, false);
			} else {
				saveSmartSwitchKeyData();
			}
		}
