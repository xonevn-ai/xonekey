...
#endif
    // Note: VK_* constants are Windows-specific, but this is safe because:
    // 1. They're only included when _WIN32 is defined
    // 2. They're just key codes, no platform-specific behavior
};
...
void checkSpelling(const bool& forceCheckVowel=false) {
    ...
    if (_index > 0 && IsIndexInWord(_index-1, _index) && CHR(_index-1) == KEY_RIGHT_BRACKET) {
        _spellingEndIndex = _index-1;
    }
    
    // Validate bounds
    if (_spellingEndIndex > 0 && _spellingEndIndex <= _index && _spellingEndIndex < MAX_BUFF) {
        j = 0;
...
void insertKey(const Uint16& keyCode, const bool& isCaps, const bool& isCheckSpelling=true) {
    // Enhanced bounds checking and safety
    if (_index >= MAX_BUFF) {
        // Save long word with bounds check
        if (IsValidIndex(0)) {
            _longWordHelper.push_back(TypingWord[0]);
        }
        //left shift with bounds checking
        for (iii = 0; iii < MAX_BUFF - 1; iii++) {
            if (IsValidIndex(iii) && IsValidIndex(iii + 1)) {
        ...
        if (SafeIncrementIndex(&_index)) {
            setKeyData(_index-1, keyCode, isCaps);
        } else {
            // Overflow protection - handle gracefully
            return;
        }
    }
    ...
    //allow d after consonant with bounds check
    if (keyCode == KEY_D && IsIndexInWord(_index - 2, _index) && IS_CONSONANT(CHR(_index - 2)))
...
void insertState(const Uint16& keyCode, const bool& isCaps) {
    ...
    if (_stateIndex >= MAX_BUFF) {
        ...
        if (IsValidIndex(_stateIndex - 1)) {
            KeyStates[_stateIndex-1] = keyCode | (isCaps ? CAPS_MASK : 0);
        }
    } else {
        // Safe write with bounds check
        if (SafeSetKeyState(_stateIndex, keyCode | (isCaps ? CAPS_MASK : 0), KeyStates, &_stateIndex)) {
            // Successfully set
        } else {
            // Overflow protection
            return;
...
Uint32 getCharacterCode(const Uint32& data) {
    // Input validation
    if (!IsValidCharacterCode(data & CHAR_MASK)) {
        return 0; // Return safe default for invalid input
    }
    ...
    if (data & MARK_MASK) { //has mark
        ...
        switch (data & MARK_MASK) {
            ...
            case MARK5_MASK:
                ...
                break;
            default:
                // Invalid mark, return safe default
                return keyCodeToCharacter(key | (capsElem == 0 ? CAPS_MASK : 0));
        }
        markElem += capsElem;
        ...
        // Bounds checking for vCodeTable and markElem
        if (vCodeTable < 0 || vCodeTable >= 5) {
            return data; // Invalid code table
        }
        
        if (_codeTable[vCodeTable].find(key) == _codeTable[vCodeTable].end())
            return data; //not found
        
        // Bounds check for markElem before array access
        const vector<Uint16>& charVector = _codeTable[vCodeTable][key];
        if (markElem < 0 || markElem >= (int)charVector.size()) {
            return data; // Invalid mark element index
        }
        
        return charVector[markElem] | CHAR_CODE_MASK;
    } else { //doesn't has mark
        // Bounds checking for vCodeTable
        if (vCodeTable < 0 || vCodeTable >= 5) {
            return data; // Invalid code table
        }
        ...
        if (data & TONE_MASK) {
            // Bounds check for capsElem
            if (capsElem < 0 || capsElem >= (int)charVector.size()) {
                return data; // Invalid index
            }
            return charVector[capsElem] | CHAR_CODE_MASK;
        } else if (data & TONEW_MASK) {
            // Bounds check for capsElem + 2
            int index = capsElem + 2;
            if (index < 0 || index >= (int)charVector.size()) {
                return data; // Invalid index
            }
            return charVector[index] | CHAR_CODE_MASK;
        } else {
            return data; //not found
        }
    }
...
void findAndCalculateVowel(const bool& forGrammar) {
    ...
    if (_index == 0 || _index > MAX_BUFF) {
        return; // No data or invalid index
    }
    
    for (iii = _index - 1; iii >= 0; iii--) {
        // Bounds check for array access
        if (!IsIndexInWord(iii, _index)) {
        ...
        if (IS_CONSONANT(CHR(iii))) {
            ...
            if (vowelCount == 0)
                VEI = iii;
            if (!forGrammar) {
                // Safe bounds check before accessing iii-1
                if (IsIndexInWord(iii-1, _index) && 
                    ((CHR(iii) == KEY_I && CHR(iii-1) == KEY_G) ||
                     (CHR(iii) == KEY_U && CHR(iii-1) == KEY_Q))) {
                    break;
                }
            }
            VSI = iii;
    ...
    // Safe bounds check
    if (IsIndexInWord(VSI - 1, _index) && IsIndexInWord(VSI, _index) &&
        CHR(VSI) == KEY_U && CHR(VSI-1) == KEY_Q) {
        if (VSI + 1 < MAX_BUFF) {
            VSI++;
            if (vowelCount > 0) {
                vowelCount--;
...
void removeMark() {
    ...
    isChanged = false;
    if (_index > 0) {
        // Bounds check for VSI and VEI
        if (VSI < MAX_BUFF && VEI < MAX_BUFF && VSI <= VEI) {
            for (i = VSI; i <= VEI && i < _index && i < MAX_BUFF; i++) {
                if (IsValidIndex(i) && TypingWord[i] & MARK_MASK) {
                    TypingWord[i] &= ~MARK_MASK;
                    isChanged = true;
                }
            }
...
void vKeyHandleEvent(const vKeyEvent& event,
    ...
    if ((IS_NUMBER_KEY(data) && capsStatus == 1)
        ...
        if (_specialChar.size() > 0) {
            ...
            if (_index > 0){
                SafeDecrementIndex(&_index);
                if (_longWordHelper.size() > 0) {
                    //right shift with bounds checking
                    for (i = MAX_BUFF - 1; i > 0; i--) {
                        if (IsValidIndex(i) && IsValidIndex(i-1)) {
                            TypingWord[i] = TypingWord[i-1];
                        }
                    }
                    if (IsValidIndex(0) && !_longWordHelper.empty()) {
