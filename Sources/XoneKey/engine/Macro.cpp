...
void initMacroMap(const Byte* pData, const int& size) {
    ...
    // Input validation
    if (!pData || size <= 0) {
        return; // Invalid input, clear map and return
    }
    
    Uint16 macroCount = 0;
    Uint32 cursor = 0;
    
    // Safe read of macro count with bounds check
    if (size >= 2) {
        memcpy(&macroCount, pData + cursor, 2);
        cursor += 2;
    } else {
        return; // Not enough data for macro count
    }
    
    // Limit macro count to prevent excessive memory usage
    const int MAX_MACRO_COUNT = 10000;
    if (macroCount > MAX_MACRO_COUNT) {
        macroCount = MAX_MACRO_COUNT;
    }
    ...
    for (int i = 0; i < macroCount; i++) {
        // Bounds check before reading macroTextSize
        ...
        // Bounds check before reading macro text
        if (cursor + macroTextSize > (Uint32)size) {
            break; // Would overflow, stop processing
        }
        
        string macroText((char*)pData + cursor, macroTextSize);
        cursor += macroTextSize;
        
        // Bounds check before reading macroContentSize
        if (cursor + 2 > (Uint32)size) {
            break; // Not enough data for content size
        }
        
        memcpy(&macroContentSize, pData + cursor, 2);
        cursor += 2;
        
        // Validate macroContentSize
        const int MAX_MACRO_CONTENT_SIZE = 4096;
        if (macroContentSize > MAX_MACRO_CONTENT_SIZE) {
            break; // Invalid size, stop processing
        }
        
        // Bounds check before reading macro content
...
void saveToFile(const string& path) {
    ...
    myfile << ";Compatible XoneKey Macro Data file for UniKey*** version=1 ***\n";
    for (std::map<vector<Uint32>, MacroData>::iterator it = macroMap.begin(); it != macroMap.end(); ++it) {
